...
**Acceptance criteria:**
- [ ] Git commit exists marking pre-migration state
- [ ] Screenshots of Setup screen + chat overlay (left alignment, right alignment, badges, emotes, fade) saved to `tasks/baseline/`
- [ ] `pnpm build` passes on baseline

**Verification:**
- [ ] Screenshots viewable and complete
...
**Acceptance criteria:**
- [ ] `src/styles/theme.css`, `src/styles/reset.css`, `src/styles/global.css` exist; `src/index.css` is deleted
- [ ] `src/main.tsx` imports `./styles/global.css`; `global.css` contains only `@import`s: theme → reset → component files, in cascade order
- [ ] No `@theme`, `@layer`, or Tailwind directives remain anywhere in `src/styles/`
- [ ] `reset.css` has no `--theme()` calls; fonts come from `var(--font-sans)`/`var(--font-mono)`
- [ ] All token names used elsewhere in the codebase are defined (`--spacing`, `--color-neutral-800/900`, `--text-*`, fonts)
- [ ] `pnpm build` passes with Tailwind still installed

**Verification:**
- [ ] Build succeeds: `pnpm build`
- [ ] Manual check: dev server shows no console CSS warnings; overlay tokens render
...
**Acceptance criteria:**
- [ ] No Tailwind utilities remain in `snippet.tsx`
- [ ] All snippet styles live in `src/styles/snippet.css`, imported in `global.css` after theme
- [ ] Copy animation still swaps Check/Copy icons on click and resets after 2s
- [ ] Disabled state (no text) renders dimmed and non-interactive

**Verification:**
- [ ] Build succeeds: `pnpm build`
- [ ] Manual check: Setup screen shows snippet with working copy button
...
**Acceptance criteria:**
- [ ] No Tailwind utilities remain in `message-renderer.tsx`
- [ ] All markdown styles live in `src/styles/markdown.css`, imported in `global.css`
- [ ] Markdown headings render at the correct relative sizes
- [ ] Emote/image-only messages keep the `h-[2lh]` sizing behavior

**Verification:**
- [ ] Build succeeds: `pnpm build`
- [ ] Manual check: send a message with markdown (heading, code, quote, image) in preview
...
**Acceptance criteria:**
- [ ] No Tailwind utilities remain in `chat.tsx`
- [ ] All chat overlay styles live in `src/styles/chat.css`, imported in `global.css`
- [ ] Chat container mask, slide-in animation, alignment (left/right), fade-out still work
- [ ] Reply block line-clamp and tint-colored last-message ring preserved

**Verification:**
- [ ] Build succeeds: `pnpm build`
- [ ] Manual check: overlay with badges, reply, both alignments, fade > 0
...
**Acceptance criteria:**
- [ ] No Tailwind utilities remain in `setup.tsx`
- [ ] All setup styles live in `src/styles/setup.css`, imported in `global.css`
- [ ] Grid collapses to one column below 768px, two columns above
- [ ] Input focus ring, button hover/shadow/disabled behaviors preserved

**Verification:**
- [ ] Build succeeds: `pnpm build`
- [ ] Manual check: Setup screen at desktop + narrow widths
...
- [ ] `pnpm lint` passes
- [ ] `pnpm build` passes
- [ ] Visual comparison vs `tasks/baseline/` screenshots shows no regressions
...
**Acceptance criteria:**
- [ ] No `tailwindcss`/`tailwind-merge` in `package.json` or lockfile
- [ ] `vite.config.ts` has no Tailwind plugin
- [ ] `cn.ts` imports only `clsx`; behavior of conditional class strings unchanged
- [ ] No `tailwind` references anywhere in `src/`, `src/styles/`, `vite.config.ts`, `index.html`
...
- [ ] `pnpm lint` passes
- [ ] `pnpm build` passes
- [ ] Overlay + Setup screen visually identical to baseline screenshots
- [ ] `dist/` CSS output size documented in the migration doc
...
- [ ] Every task above completed and checked
- [ ] `pnpm lint` + `pnpm build` green on final state
- [ ] No visual regressions vs baseline
- [ ] Migration doc updated with results
